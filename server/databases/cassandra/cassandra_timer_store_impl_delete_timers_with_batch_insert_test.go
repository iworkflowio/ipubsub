...
func TestDeleteTimersUpToTimestampWithBatchInsert_Basic(t *testing.T) {
	...
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-to-delete-1").Scan(&count1)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, count1, "timer-to-delete-1 should be deleted")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-to-delete-2").Scan(&count2)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, count2, "timer-to-delete-2 should be deleted")

	// Verify timer outside range still exists
	var countOutside int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-outside-range").Scan(&countOutside)
	require.NoError(t, scanErr)
	...
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer-1").Scan(&countNew1)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, countNew1, "new-timer-1 should be inserted")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer-2").Scan(&countNew2)
	require.NoError(t, scanErr)
	...
	selectQuery := `SELECT timer_callback_url, timer_payload, timer_retry_policy, timer_callback_timeout_seconds FROM timers 
	                WHERE timer_namespace = ? AND timer_id = ? ALLOW FILTERING`

	scanErr = store.session.Query(selectQuery, namespace, "new-timer-1").
	...
	scanErr = store.session.Query(selectQuery, namespace, "new-timer-2").
		Scan(&dbCallbackUrl, &dbPayload, &dbRetryPolicy, &dbTimeout)
	require.NoError(t, scanErr)
...
func TestDeleteTimersUpToTimestampWithBatchInsert_EmptyDelete(t *testing.T) {
	...
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_namespace = ? AND timer_id = ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-outside").Scan(&countOriginal)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, countOriginal, "original timer should still exist")

	// Verify new timer was inserted
	var countNew int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer").Scan(&countNew)
	require.NoError(t, scanErr)
...
func TestDeleteTimersUpToTimestampWithBatchInsert_NoInserts(t *testing.T) {
	...
	var count1, count2 int
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_namespace = ? AND timer_id = ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-delete-only-1").Scan(&count1)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, count1, "timer-delete-only-1 should be deleted")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-delete-only-2").Scan(&count2)
	require.NoError(t, scanErr)
...
func TestDeleteTimersUpToTimestampWithBatchInsert_ShardVersionMismatch(t *testing.T) {
	...
	var countOriginal int
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_namespace = ? AND timer_id = ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-version-test").Scan(&countOriginal)
	require.NoError(t, scanErr)
	...
	var countNew int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer-version-test").Scan(&countNew)
	require.NoError(t, scanErr)
...
func TestDeleteTimersUpToTimestampWithBatchInsert_ConcurrentOperations(t *testing.T) {
	...
	var originalTimerCount int
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_execute_at <= ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, now.Add(6*time.Minute)).Scan(&originalTimerCount)
	require.NoError(t, scanErr)
	assert.Equal(t, 5, originalTimerCount, "All original timers should still exist")

	// Verify that new timers were inserted
	var newTimerCount int
	countQuery = `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_execute_at > ? ALLOW FILTERING`
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, now.Add(6*time.Minute)).Scan(&newTimerCount)
	require.NoError(t, scanErr)
	assert.Equal(t, 3, newTimerCount, "All new timers should be inserted")
}
...
func TestDeleteTimersUpToTimestampWithBatchInsert_ShardVersionChanged(t *testing.T) {
	...
	var countOriginal int
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_namespace = ? AND timer_id = ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-shard-change-test").Scan(&countOriginal)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, countOriginal, "original timer should be deleted")

	// Verify new timer was inserted
	var countNew int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer-shard-change-test").Scan(&countNew)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, countNew, "new timer should be inserted")
}
...
func TestDeleteTimersUpToTimestampWithBatchInsert_InvalidPayloadSerialization(t *testing.T) {
	...
	var countOriginal int
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_namespace = ? AND timer_id = ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "timer-invalid-payload-test").Scan(&countOriginal)
	require.NoError(t, scanErr)
...
func TestDeleteTimersUpToTimestampWithBatchInsert_LargeTimestamp(t *testing.T) {
	...
	var count4, count5 int
	countQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? AND timer_namespace = ? AND timer_id = ? ALLOW FILTERING`
	scanErr := store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "large-range-timer-3").Scan(&count4)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, count4, "timer-3 should still exist")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "large-range-timer-4").Scan(&count5)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, count5, "timer-4 should still exist")

	// Verify new timer was inserted
	var countNew int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-large-range-timer").Scan(&countNew)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, countNew, "new timer should be inserted")

	// Verify deleted timers are gone
	var count1, count2, count3 int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "large-range-timer-0").Scan(&count1)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, count1, "timer-0 should be deleted")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "large-range-timer-1").Scan(&count2)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, count2, "timer-1 should be deleted")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "large-range-timer-2").Scan(&count3)
	require.NoError(t, scanErr)
	assert.Equal(t, 0, count3, "timer-2 should be deleted")
}
...
func TestDeleteTimersUpToTimestampWithBatchInsert_InsertInDeleteRange(t *testing.T) {
	...
	var countNew1, countNew2 int
	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer-in-range-1").Scan(&countNew1)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, countNew1, "new-timer-in-range-1 should be inserted")

	scanErr = store.session.Query(countQuery, shardId, databases.RowTypeTimer, namespace, "new-timer-in-range-2").Scan(&countNew2)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, countNew2, "new-timer-in-range-2 should be inserted")

	// Verify new timer data is correct and contains expected payload/retry policy
	var dbCallbackUrl, dbPayload, dbRetryPolicy string
	var dbTimeout int32
	var dbExecuteAt time.Time
	selectQuery := `SELECT timer_callback_url, timer_payload, timer_retry_policy, timer_callback_timeout_seconds, timer_execute_at FROM timers 
	                WHERE timer_namespace = ? AND timer_id = ? ALLOW FILTERING`

	// Check newTimer1
	scanErr = store.session.Query(selectQuery, namespace, "new-timer-in-range-1").
		Scan(&dbCallbackUrl, &dbPayload, &dbRetryPolicy, &dbTimeout, &dbExecuteAt)
	require.NoError(t, scanErr)
	assert.Equal(t, "https://example.com/new-callback-1", dbCallbackUrl)
	assert.Contains(t, dbPayload, "insert-in-range")
	assert.Equal(t, int32(45), dbTimeout)
	assert.WithinDuration(t, now.Add(6*time.Minute), dbExecuteAt, time.Second)

	// Check newTimer2
	scanErr = store.session.Query(selectQuery, namespace, "new-timer-in-range-2").
		Scan(&dbCallbackUrl, &dbPayload, &dbRetryPolicy, &dbTimeout, &dbExecuteAt)
	require.NoError(t, scanErr)
	assert.Equal(t, "https://example.com/new-callback-2", dbCallbackUrl)
	assert.Contains(t, dbRetryPolicy, "linear")
	assert.Equal(t, int32(60), dbTimeout)
	assert.WithinDuration(t, now.Add(7*time.Minute), dbExecuteAt, time.Second)
	...
	rangeQuery := `SELECT COUNT(*) FROM timers WHERE shard_id = ? AND row_type = ? 
	               AND timer_execute_at >= ? AND timer_execute_at <= ? ALLOW FILTERING`
	scanErr = store.session.Query(rangeQuery, shardId, databases.RowTypeTimer,
		now.Add(4*time.Minute), now.Add(9*time.Minute)).Scan(&totalInRange)
	require.NoError(t, scanErr)
	assert.Equal(t, 2, totalInRange, "Only the 2 newly inserted timers should exist in the range")

	// Double-check by verifying the specific timer IDs that exist in the range
	iter := store.session.Query(`SELECT timer_id FROM timers WHERE shard_id = ? AND row_type = ? 
	                              AND timer_execute_at >= ? AND timer_execute_at <= ? ALLOW FILTERING`,
		shardId, databases.RowTypeTimer, now.Add(4*time.Minute), now.Add(9*time.Minute)).Iter()

	var foundTimerIds []string
	var timerId string
	for iter.Scan(&timerId) {
		foundTimerIds = append(foundTimerIds, timerId)
	}
	require.NoError(t, iter.Close())

	assert.Len(t, foundTimerIds, 2, "Should find exactly 2 timers in the range")
	assert.Contains(t, foundTimerIds, "new-timer-in-range-1")
	assert.Contains(t, foundTimerIds, "new-timer-in-range-2")
	assert.NotContains(t, foundTimerIds, "timer-to-delete-1")
	assert.NotContains(t, foundTimerIds, "timer-to-delete-2")
}
