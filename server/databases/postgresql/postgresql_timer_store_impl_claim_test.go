...
func TestClaimShardOwnership_NilMetadata(t *testing.T) {
	...
	var dbMetadata *string
	query := "SELECT shard_metadata FROM timers WHERE shard_id = $1 AND row_type = $2 AND timer_execute_at = $3 AND timer_uuid_high = $4 AND timer_uuid_low = $5"
