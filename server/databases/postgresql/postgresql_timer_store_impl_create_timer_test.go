...
func TestCreateTimer_ConcurrentCreation(t *testing.T) {
	...
	var dbCount int
	query := `SELECT COUNT(*) FROM timers 
	          WHERE shard_id = $1 AND row_type = $2 AND timer_namespace = $3`
...
func TestCreateTimer_DuplicateTimerOverwrite(t *testing.T) {
	...
	require.NoError(t, scanErr)
	assert.Equal(t, 1, count1, "After same UUID + same execute_at: PostgreSQL should have 1 timer")
	...
	// 5. Verify count = 1 (PostgreSQL should still have 1 timer due to unique constraint)
	var count2 int
	scanErr = store.db.QueryRow(countQuery, shardId, databases.RowTypeTimer, namespace, timerId).Scan(&count2)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, count2, "After same UUID + different execute_at: PostgreSQL should have 1 timer")
	...
	// 7. Verify count = 1 (PostgreSQL should still have 1 timer due to unique constraint)
	var count3 int
	scanErr = store.db.QueryRow(countQuery, shardId, databases.RowTypeTimer, namespace, timerId).Scan(&count3)
	require.NoError(t, scanErr)
	assert.Equal(t, 1, count3, "After different UUID + different execute_at: PostgreSQL should have 1 timer")
...
func TestCreateTimerNoLock_DuplicateTimerOverwrite(t *testing.T) {
	...
	updatedTimer := &databases.DbTimer{
		...
		TimerUuid:              databases.GenerateTimerUUID(namespace, "duplicate-timer-nolock"), // Same stable UUID
		Namespace:              namespace,
...
func TestCreateTimerNoLock_ConcurrentCreation(t *testing.T) {
	...
	var dbCount int
	query := `SELECT COUNT(*) FROM timers 
	          WHERE shard_id = $1 AND row_type = $2 AND timer_namespace = $3 AND timer_id LIKE $4`
